...
class SKU:
    ...
    def calculate_order(self):
        # TODO заказ должен быть в кол-ве коробок
        if type(self.box_weight) == str:
            print(
                f"ОШИБКА. В бланке завода в колонке с весом ящика указана формула, а должно быть число. " +
                f"Строка - {self.num_row}."
            )
        ...
        res = round(weight / self.box_weight)
        self.res_order = res

    def calculate_weight(self):
        return round(self.res_order * self.box_weight, 4)
    ...
    def __str__(self):
        return f"{self.name_1c}: {self.order} => {self.article}: {self.name_manufacture}. ORDER: {self.res_order}\n"
