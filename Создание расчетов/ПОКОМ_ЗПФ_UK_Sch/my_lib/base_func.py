...
def add_data_from_old_file(file_dv_old, table):
    ...
    all_columns = {
        ...
        'orders_is_on_the_way_last': {}, # заказ в пути (в коробках)
    ...
    for row in range(1, max_row + 1):
        ...
        for col in range(1, max_col + 1):
            ...
            if cell_value == 'сроки':
                ...
                columns_val.append(col)
            if cell_value in ('заказ в пути', 'заказ в дороге'):
            ...
            if cell_value == 'заказ кор.':
                if ws.cell(row=row + 1, column=col).value:
                    all_columns['orders_is_on_the_way_last'].update({ws.cell(row=row + 1, column=col).value: col})
                    columns_dict.append(col)
            if cell_value in ('ср', 'ср нов'):
                if ws.cell(row=row + 1, column=col).value:
                    all_columns['average_values'].update({ws.cell(row=row + 1, column=col).value: col})
                    columns_dict.append(col)
            if cell_value in ('комментарии', 'коментарии', 'комментарий', 'коментарий'):
                all_columns['comments'] = col
                columns_val.append(col)
            if cell_value == 'крат кор':
                all_columns['boxes'] = col
                columns_val.append(col)
            ...
            if all(all_columns.values()) and current_sku and col in columns_val:
                ...
                elif col in all_columns['orders_is_on_the_way_last'].values():
                    value = cell_value
                    if value == '#REF!':
                        value = '#Н/Д'# 0
                    key_or_in_way_last = list(filter(lambda x: x[1] == col, all_columns['orders_is_on_the_way_last'].items()))[0][0]
                    current_obj.__dict__['orders_is_on_the_way_last'][key_or_in_way_last] = value
                    table.COLUMNS['orders_is_on_the_way_last'][1].update({key_or_in_way_last: None})
